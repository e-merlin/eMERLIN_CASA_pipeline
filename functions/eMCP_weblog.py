...
def weblog_header(wlog, section, project):
    ...
    wlog.write(
        '<link rel="stylesheet" type="text/css" href="{0}eMCP.css"/>\n'.format(
            weblog_link))
    wlog.write('<head>\n')
    wlog.write('<title>{} - eMCP</title>\n'.format(project))
    wlog.write('<link rel="icon" href="{0}{1}">\n'.format(
        weblog_link, 'eMCP_logo.png'))
    wlog.write('</head>\n')
    wlog.write('<body>\n')
    wlog.write('<div class="header-img">\n')
    wlog.write(
        '<a href="./index.html"><img src="{0}{1}" width="40"/></a>\n'.format(
            weblog_link, 'eMCP_logo.png'))
...
def weblog_index(msinfo):
    ...
    #------------------------------------------
    wlog.write(
        '<table class="table1" bgcolor="#eeeeee" border="3px" cellspacing = "0" cellpadding = "4px" style="width:40%">\n'
    )
    ...
    wlog.write('<tr><td>Start </td>  <td> {0} </td>\n'.format(
        msinfo['t_ini'].strftime("%Y-%m-%d %H:%M")))
    wlog.write('<tr><td>End </td>    <td> {0} </td>\n'.format(
        msinfo['t_end'].strftime("%Y-%m-%d %H:%M")))
    ...
    wlog.write(
        '<tr><td>Integration time </td>             <td> {0}s</td>\n'.format(
            msinfo['int_time']))
    wlog.write(
        '<tr><td>Frequency </td>  <td> {0:5.2f} - {1:5.2f} GHz </td>\n'.format(
            msinfo['freq_ini'], msinfo['freq_end']))
    wlog.write('<tr><td>Num. spw </td>             <td> {0} </td>\n'.format(
        msinfo['num_spw']))
    wlog.write(
        '<tr><td><abbr title="Number of channels per spw after  averaging. The raw data may provide up to 512 channels">Channels/spw<sup>*</sup></abbr></td><td> {0} </td>\n'
        .format(msinfo['nchan']))
    wlog.write('<tr><td>Channel width </td><td> {0:6.2f} MHz</td>\n'.format(
        msinfo['chan_res'] * 1000))
...
def weblog_obssum(msinfo):
    ...
    write_link_txt(wlog, listobs_file,
                   'Summary of current observation (listobs)')
    all_listobs = [
        os.path.basename(l)
    ...
    if len(all_listobs) > 0:
        ...
        for listobs in all_listobs:
            write_link_txt(wlog, info_link + listobs,
                           os.path.basename(listobs))
    wlog.write('<h3>Sources:</h3>\n')
    sepfile = info_link + 'source_separations.txt'
    if msinfo['sources']['targets'] != '':
        wlog.write(
            '<table class="table1" bgcolor="#eeeeee" border="3px" cellspacing = "0" cellpadding = "4px" style="width:30%">\n'
        )
        wlog.write(
            '<tr><td><b>Target</b> </td><td><b>Phase cal</b></td><td><b>Separation [deg]</b></td>\n'
        )
        ...
        for s1, s2 in zip(msinfo['sources']['targets'].split(','),
            ...
            if s1 not in msinfo['sources']['mssources'].split(','):
                ...
                try:
                    ...
                    try:
                        separation = '{0:5.2f}'.format(separations[s2 + '-' +
                                                                   s1])
                    except:
                        separation = '0.0'
    ...
    wlog.write(
        '<tr><td><b>Source</b> </td><td> <b>Intent</b><td><b>MJD range</b></td>\n'
    )
    for source in msinfo['sources']['mssources'].split(','):
        ...
        wlog.write('<tr><td>{0} </td><td> {1}</td> <td>{2}</td>\n'.format(
            source, msinfo['sources']['source_intent'][source], mjd_range))
    wlog.write('</table><br>\n')
    ...
    if missing_sources != '':
        wlog.write(
            '*Sources specified in the inputs file but not in MS: {0}\n'.
    ...
    try:
        elev_plot = glob.glob(weblog_dir +
                              'plots/plots_observation/{0}_elevation.png'.
                              format(msinfo['msfilename']))
        wlog.write(
            '<a href = ".{0}"><img style="max-width:600px" src=".{0}"></a><br>\n'
            .format(elev_plot[0]))
    except:
        pass
    ...
    try:
        ...
        for u in uvcov:
            wlog.write(
                '<a href = ".{0}"><img style="max-width:700px" src=".{0}"></a><br>\n'
                .format(u))
            wlog.write('<hr>\n')
    except:
        pass
...
def table_colors(steps, step, prev_steps):
    ...
    if steps[step][0] == 0:
        ...
        if prev_steps != []:
            latest_prev = max([
                datetime.datetime.strptime(steps[stepi][0], t_fmt)
...
def table_steps(eMCP):
    ...
    table_txt += (
        '<table class="table1" bgcolor="#eeeeee" border="3px" cellspacing = "0" cellpadding = "4px" style="width:80%">\n'
    )
    ...
    for step, step_info in eMCP['steps'].items():
        ...
        table_txt += (
            '<td align="center" bgcolor={0}><small>{1}</small></td>\n'.format(
                color, status))
...
def write_fluxscale(wlog, msinfo):
    p = glob.glob(calib_dir +
                  '{0}_fluxscale.png'.format(msinfo['msfilename']))[0]
    wlog.write(
...
def weblog_calibration(eMCP):
    ...
    if os.path.isfile('./weblog/calib/caltables.pkl'):
        ...
        for calstep in all_calsteps:
            ...
            try:
                if calstep == 'fluxscale' and os.path.isfile(
                        calib_dir + 'allcal_ap.G1_fluxes.txt'):
                    wlog.write('<h4>{}</h4>\n'.format('fluxscale'))
                    write_fluxscale(wlog, msinfo)
                    wlog.write('\n<hr>\n')
                else:
                    wlog.write('<h4>{}</h4>\n'.format(
                        caltables[calstep]['name']))
                    wlog.write(
                        '<table cellspacing = "20" cellpadding = "4px" style="width:40%">\n<tr> <td valign="top">\n'
                    )
                    write_caltable(caltables[calstep], wlog)
                    all_plots = np.sort(
                        glob.glob('./weblog/plots/caltables/*{}*.png'.format(
                            calstep)))
                    for p in all_plots:
                        wlog.write(
                            '<td><a href = ".{0}"><img style="max-width:700px" src=".{0}"></a></td>\n'
    ...
    try:
        ...
        wlog.write(
            'List of tables and apply parameters used to correct each source:')
...
def weblog_flagstats(msinfo):
    ...
    for step in flagstats_steps:
        try:
            ...
            all_plots = np.sort(
                glob.glob(
                    './weblog/plots/plots_flagstats/*{}.png'.format(step)))
            if len(all_plots) > 0:
                wlog.write('<a href=".{0}" target="_blank">{1}</a> (Total: '
                           '{2:3.1f}%. Increase: {3:3.1f}%)<br>\n'.format(
                               all_plots[1], step, perc_flagged, diff_flagged))
                wlog.write(
                    '<a href = ".{0}"><img style="max-width:1200px" src=".{0}"></a><br>\n'
                    .format(all_plots[0]))
...
def weblog_images(eMCP):
    ...
    if check_any_image(eMCP):
        ...
        wlog.write(
            '<br><div style="max-width:800px; word-wrap:break-word;">{0}</div>'
            .format(note_text))
    else:
...
def write_img_zoom(img):
    ...
    <div class="imageBox">
      <div class="imageInn">
        <img src="{0}" alt="Image" style="max-width:600px">
      </div>
      <div class="hoverImg">
        <img src="{1}" alt="Zoom" style="max-width:800px">
      </div>
    </div>
...
def show_image(eMCP, wlog, i):
    ...
    wlog.write('<hr>\n')
    wlog.write(
        '<h3 id="{0}">{0} <a href="#top"> <small>(up)</small></a></h3>\n'.
        format(target))
    #    wlog.write('<a href="#images"> (up)</a><br>\n')

    wlog.write(
        '<table class="table1" bgcolor="#eeeeee" border="3px" cellspacing = "0" cellpadding = "4px" style="width:70%">\n'
    )
    ...
    wlog.write('</tr>')
    #    wlog.write('<tr><td><b>{0}</b> (Phasecal) ' \
    #               'Peak: {1:5.1f} mJy ' \
    #               '(rms: {2:5.3f} mJy/b)</td><td>Zoom</td></tr>\n'.format(phscal,
    #                                                               peak_phscal*1000.,
    #                                                               noise_phscal*1000.))
...
def weblog_download(msinfo):
    ...
    #------------------------------------------
    wlog.write(
        'This tar file contains the MS and all the plots in the weblog:<br>')
